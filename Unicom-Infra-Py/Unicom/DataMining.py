...
class DataMining:
    ...
    def arpu_call_sum_liner(self, df: DataFrame):
        Q1 = df['流量使用量'].quantile(0.25)
        Q3 = df['流量使用量'].quantile(0.75)
        IQR = Q3 - Q1
        lower_bound = Q1 - 1.5 * IQR
        upper_bound = Q3 + 1.5 * IQR
        df_filtered = df.drop(df[(df['流量使用量'] >= lower_bound) & (df['流量使用量'] <= upper_bound)].index)
...
if __name__ == '__main__':
    mining = DataMining()
    a = DataAnalysis()
    df = pd.read_csv("F:/Dev/Unicom-Moniter/Unicom-Moniter/mysql-export/part-r-00011", sep=',',
                     encoding='utf-8', names=a.column_names, dtype=a.dtypes,
                     on_bad_lines="skip")
    df = df.dropna(subset=['终端品牌'])
    mining.arpu_call_sum_liner(df) #arpi权重
    df = mining.groupBy_features(df) #分许，
    mining.net_consume_diff(df)
    a.column_names.append('换手机')
    a.column_names.append('换手机时间')
    a.dtypes['换手机'] = 'int32'
